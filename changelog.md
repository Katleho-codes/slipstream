...
- swapped Resend for Nodemailer SMTP delivery (`email.service.ts` now uses a nodemailer transporter); `.env`/.env.example keys replaced (`RESEND_*` → `MAIL_HOST`/`MAIL_PORT`/`MAIL_USER`/`MAIL_PASS`/`MAIL_FROM_EMAIL`/`MAIL_FROM_NAME`) for Mailtrap
